...
This project provides a practical alternative to DFS-R for Samba Active Directory environments.

Changes made to SYSVOL on the authoritative domain controller are replicated automatically to secondary domain controllers within seconds using `lsyncd` and `rsync`.

A nightly reconciliation job provides an additional safety net.

## Tested Environment

- Domain: `ecs.kinghorndesign.com`
...
ad1 (authoritative SYSVOL source)
  └─ lsyncd monitors:
     /var/lib/samba/sysvol/ecs.kinghorndesign.com/
        └─ rsync over SSH
           ├─ ad2
           └─ ad3
...
Consider adding an MIT License if you wish to formalize reuse terms.
